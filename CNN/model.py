...
# from tensorflow.keras.layers import CuDNNLSTM as LSTM # only for GPU
...
class Shared_Model:
    def __init__(self, input_shape, action_space, lr, optimizer, model="Dense"):
        ...
        # Shared ResNet layers:   
            ...
            # Add one-dimensional ResNet model
            X_res = X
            for i in range(3):
                X_res = Conv1D(filters=32, kernel_size=3, padding="same", activation="relu")(X_res)
                X_res = Conv1D(filters=32, kernel_size=3, padding="same", activation=None)(X_res)
                print(X.shape)
                print(X_res.shape)
                X = Add()([X, X_res])
                print(X.shape)
