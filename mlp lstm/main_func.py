...
class CustomEnv:
    ...
    def step(self, action, prediction):
        ...
        # ablation study
        # prediction = 1.0
        # prediction = 1.0/(1+np.exp(-5.0*(prediction-0.5)))
        # prediction = -0.5*np.cos(np.pi*prediction)+0.5
        ...
        if action == 0: # Hold
            ...
            self.crypto_bought = self.balance / current_price * prediction
            self.crypto_bought *= (1-self.fees) # substract fees
            self.balance -= self.crypto_bought * current_price
            self.crypto_held += self.crypto_bought
            # print(prediction, self.crypto_bought, self.balance, self.crypto_held)
            ...
            self.crypto_sold = self.crypto_held * prediction
            self.crypto_sold *= (1-self.fees) # substract fees
            self.balance += self.crypto_sold * current_price
            self.crypto_held -= self.crypto_sold
            # print(prediction, self.crypto_sold, self.balance, self.crypto_held)
...
if __name__ == "__main__":            
    ...
    df_nomalized = pd.read_csv('eth_1h_normalized.csv')
    ...
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=1e-5, epochs=10, optimizer=RMSprop, batch_size=128, model="CNN", depth=depth, comment="Normalized")
    # print(train_df.columns)
    # print(len(train_df_nomalized), train_df_nomalized.shape)
    # print(train_df_nomalized.columns)
    # train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 32, training_batch_size=2000, visualize=False, EPISODES=50000)
    ...
    # can run 
    test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 16, \
                         visualize=False, test_episodes=1000, folder="./2023_04_04_11_46_Crypto_trader", \
                            name="1339.86_Crypto_trader", comment="3 months")
