...
from tensorflow.keras.layers import Input, Dense, Flatten, Conv1D, MaxPooling1D, LeakyReLU, LSTMCell, RNN#, LSTM
from tensorflow.compat.v1.keras.layers import CuDNNLSTM as LSTM # only for GPU
from tensorflow.keras import backend as K
# from kerasbeats import NBeatsModel, NBeats
...
class Shared_Model:
    def __init__(self, input_shape, action_space, lr, optimizer, model="Dense"):
        ...
        # Shared LSTM layers:
            ...
            X = LSTM(256, return_sequences=True)(X_input) # (32, 100, 512)
            # print(X.shape) 
            ...
            X = RNN(lstm_cell, return_state=True)(X_input)
            X = Dense(64)(X)

        # Shared Dense layers:
        elif model=="Nbeats":
            nbeats = NBeatsModel(lookback=100)
            nbeats.build_layer()
            X = nbeats.model_layer(X_input)
    ...
    def actor_predict(self, state):
        out = self.Actor.predict(state) # (32, 3)
        # print(out)
        # print(out.shape)
        return out
