...
class CustomAgent:
    ...
    def replay(self, states, actions, rewards, predictions, dones, next_states):
        ...
        # reduce_lr = callbacks.LearningRateScheduler(self.scheduler,verbose=1)
...
if __name__ == "__main__":            
    ...
    df = pd.read_csv('new_data/eth_1h.csv')
    df_nomalized = pd.read_csv('new_data/eth_1h_normalized.csv')

    depth = len(list(df.columns[1:])) # OHCL + indicators without Date

    lookback_window_size = 50
    test_window = 720*3 # 6 months
    ...
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.0001, epochs=20, optimizer=Adam, batch_size=512, model="MyModel", depth=depth, comment="Normalized")
    ...
    test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 16, visualize=False, test_episodes=1000, folder="2023_03_26_23_10_Crypto_trader", name="1975.02_Crypto_trader", comment="3 months")
