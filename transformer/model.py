...
from tensorflow.keras.layers import Input, Dense, Flatten, Conv1D, MaxPooling1D, Bidirectional, Dropout, GlobalAveragePooling1D#, LSTM
...
class Shared_Model:
    def __init__(self, input_shape, action_space, lr, optimizer, model="Dense"):
        ...
        # Shared LSTM layers:
            ...
            # print(X_input.shape)
            # X = Bidirectional(GRU(512, return_sequences=True))(X_input)
            # X = GRU(512, return_sequences=True)(X_input)
            # print(X.shape)
            # X = GRU(256)(X)
            print(X.shape)
